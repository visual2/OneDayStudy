...
:checked 선택자로 input[type="radio"]의 선택 상태를 판별할 수 있다.
~ 선택자를 이용하여 뒤에 있는 태그를 선택하고 조작할 수 있다.
label을 클릭하면, for과 일치하는 id 값을 가진 input에 focusing 된다.
input은 display:none 상태에서도 작동한다.
